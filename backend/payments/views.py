...
def settlements_page(request):
    ...
    week_values = (
        settlements.values("week_start", "week_end")
        .distinct()
        .order_by("-week_start")
    )
    week_options = [
    ...
    if (
        selected_week_start
        and selected_week_start.weekday() == 0
        and selected_week_start in valid_starts
    ):
        settlements = settlements.filter(week_start=selected_week_start)
        selected_week = next(
            (w for w in week_options if w["week_start"] == selected_week_start),
            None,
        )
...
def producer_payment_report_csv(request):
    ...
    for record in records:
        ...
        settlement_status = "SETTLED" if settlement else "UNSETTLED"
        settlement_batch_id = settlement.id if settlement else ""
        settlement_batch_status = settlement.status if settlement else ""
        customer_reference = (record.customer_reference or "").strip()
        customer_identifier = (
            f"cust-{customer_reference[-4:]}" if customer_reference else "cust-unknown"
        )
