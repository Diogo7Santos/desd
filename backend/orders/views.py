...
def _producer_lead_time_days(producer) -> int:
    profile = _safe_producer_profile(producer)
    if not profile:
        return 2
    return max(2, getattr(profile, "lead_time_days", 2) or 2)


def _producer_delivery_field_names(producer):
    return {
        "date": f"producer_{producer.id}_delivery_date",
        "notes": f"producer_{producer.id}_delivery_notes",
    }


def _producer_earliest_delivery_date(producer):
    return timezone.localdate() + timedelta(days=_producer_lead_time_days(producer))


def _build_checkout_producer_sections(items_by_producer):
    sections = []
    for producer, items in items_by_producer.items():
        field_names = _producer_delivery_field_names(producer)
        profile = _safe_producer_profile(producer)
        earliest_delivery_date = _producer_earliest_delivery_date(producer)
        sections.append(
            {
                "producer": producer,
                "items": items,
                "business_name": profile.business_name if profile else producer.username,
                "contact_name": profile.contact_name if profile else (producer.get_full_name() or producer.username),
                "lead_time_days": _producer_lead_time_days(producer),
                "earliest_delivery_date": earliest_delivery_date,
                "earliest_delivery_date_input": earliest_delivery_date.isoformat(),
                "date_field_name": field_names["date"],
                "notes_field_name": field_names["notes"],
            }
        )
    return sections


def _parse_producer_delivery_preferences(
    request,
    items_by_producer,
    *,
    ...
    for producer in items_by_producer:
        field_names = _producer_delivery_field_names(producer)
        delivery_date_raw = (request.POST.get(field_names["date"]) or "").strip()
        delivery_notes = (request.POST.get(field_names["notes"]) or "").strip()

        if delivery_date_raw:
            try:
                producer_delivery_date = datetime.strptime(delivery_date_raw, "%Y-%m-%d").date()
            except ValueError as exc:
                raise ValueError(f"Invalid delivery date format for {producer.username}.") from exc
        elif fallback_delivery_date is not None:
            producer_delivery_date = fallback_delivery_date
        else:
            raise ValueError(f"Please choose a delivery date for {producer.username}.")

        min_delivery_date = _producer_earliest_delivery_date(producer)
        if producer_delivery_date < min_delivery_date:
            profile = _safe_producer_profile(producer)
            business_name = profile.business_name if profile else producer.username
            raise ValueError(
                f"{business_name} requires at least {_producer_lead_time_days(producer)} days notice. "
                f"Earliest available date is {min_delivery_date}."
            )

        delivery_preferences[producer.id] = {
...
def _create_order_and_payment_records(
    *,
    ...
    producer_delivery_preferences=None,
    history_note,
):
    if not line_items:
    ...
    producer_delivery_preferences = producer_delivery_preferences or {}
    for line_item in line_items:
        product = line_item["product"]
        quantity = line_item["quantity"]
        unit_price = product.price
        producer_delivery = producer_delivery_preferences.get(product.producer_id, {})
        ...
        OrderItem.objects.create(
            ...
            quantity=quantity,
            producer_delivery_date=producer_delivery.get("delivery_date", delivery_date),
            producer_delivery_notes=producer_delivery.get("delivery_notes", ""),
        )
...
def checkout(request):
    ...
    if len(items_by_producer) == 1:
        single_producer = next(iter(items_by_producer))
        min_delivery_date = _producer_earliest_delivery_date(single_producer)
    producer_delivery_sections = _build_checkout_producer_sections(items_by_producer)

    context = {
        "cart": cart,
        "items_by_producer": items_by_producer,
        "producer_delivery_sections": producer_delivery_sections,
...
def place_order(request):
    ...
    delivery_date_str = (request.POST.get("delivery_date") or "").strip()
    ...
    if delivery_date_str:
        ...
        if delivery_date < min_delivery:
            ...
            return redirect("orders:checkout")
    elif len(items_by_producer) == 1:
        messages.error(request, "Please choose a delivery date.")
        return redirect("orders:checkout")
    ...
    try:
        producer_delivery_preferences = _parse_producer_delivery_preferences(
            request,
            items_by_producer,
            fallback_delivery_date=delivery_date,
        )
    except ValueError as exc:
        messages.error(request, str(exc))
        return redirect("orders:checkout")

    overall_delivery_date = min(
        detail["delivery_date"] for detail in producer_delivery_preferences.values()
    )
    ...
    order = _create_order_and_payment_records(
        ...
        delivery_instructions=delivery_instructions,
        producer_delivery_preferences=producer_delivery_preferences,
...
def producer_dashboard(request):
    ...
    orders_data = []
    for order in orders:
        grouped_items = order.get_items_by_producer().get(
            request.user,
            {
                "items": list(order.items.filter(producer=request.user)),
        ...
        orders_data.append(
            {
                ...
                "subtotal": grouped_items["subtotal"],
                "delivery_date": grouped_items["delivery_date"] or order.delivery_date,
                "producer_delivery_notes": grouped_items["delivery_notes"],
                "payment_status": _payment_status_for_order(order),
                "customer_summary": _build_customer_summary(order.customer),
            }
        )
